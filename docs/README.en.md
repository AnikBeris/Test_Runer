...
2. The author provides no guarantees, explicit or implied, regarding the accuracy, completeness, or suitability of this image for any particular purposes. 
3. The author is not liable for any damages, including but not limited to direct, indirect, incidental, consequential, or special damages arising from the use or inability to use this image or its accompanying documentation, even if advice on the possibility of such damages was provided in advance.

4. By using this image, you acknowledge and assume all risks associated with its use. Additionally, you agree that the author cannot be held responsible for any issues or consequences arising from its application.
...
<h4 align="center"> 
Donations are greatly appreciated, no matter how small. Thank you so much. 😌 
</h1>
...
<h2 align="center">
  <a href="#-table-of-contents"> ⬆️ Up </a> 
</h2>
...
- **x86 / i386:** Widely used in desktop and laptop computers. It is extensively supported by various operating systems and applications, including but not limited to `Windows, macOS and Linux`.
...
- **armv7 / arm / arm32:** Serves as the architecture for older mobile and embedded devices. It is still widely used in devices such as `Orange Pi Zero LTS, Orange Pi PC Plus, Raspberry Pi 2`, and others.

- **armv6 / arm / arm32:** Focused on very old embedded devices. While less common, it is still applied, for example, in `Raspberry Pi 1, Raspberry Pi Zero/Zero W`.

- **armv5 / arm / arm32:** An older architecture mainly related to early embedded systems. Rare today but may be found in legacy devices like initial versions of `Raspberry Pi` and some old smartphones.
...
- Searching across all incoming connections and clients
- Theme: `dark/light`
...
- Traffic statistics, traffic limits, expiration time
- Customizable Xray configuration templates
- Support for HTTPS access panel (`custom domain + SSL certificate`)
- One-click request for `SSL certificate` and its automatic renewal
- For more advanced configuration options, see the panel
- Fixed `API routes` (user settings created through `API`)
- Support for modifying configurations through various options available in the panel
...
The author provides no guarantees, explicit or implied, regarding the accuracy, completeness, or suitability of this image for any particular purposes. The author is not liable for any damages, including but not limited to direct, indirect, incidental, consequential, or special damages arising from the use or inability to use this image or its accompanying documentation, even if advice on the possibility of such damages was provided in advance.

By using this image, you acknowledge and assume all risks associated with its use. Additionally, you agree that the author cannot be held responsible for any issues or consequences arising from its application.
...
- Domain name must be directed (set in DNS) to the current server via `Cloudflare`



## How to Obtain a Global API Key from Cloudflare:
...
4. You may need to re-authenticate. After that, the key will be displayed (see screenshot below):
...
1. System Update
Before starting the installation, ensure your system is up to date. Execute the following commands:
...
Answer `y` if you want to set the port yourself, or `n` for the script to set it automatically. 
Do not use standard ports like `22, 80, 8080`. We recommend using another port, for instance, `8181`.
...
After completing these steps, the panel installation will be finished, and you can connect through a browser to the panel with the following path:

```sh
http://YOUR_SERVER_IP_ADDRESS:PORT/WebBasePath (e.g., http://192.168.0.10:40608/vpkPI6ex9ajesDX)
```

After entering the address in the browser, you will reach the login window for the panel, where you need to enter the username and password provided earlier by the script.
...
After finishing, click `Save` and `Restart Panel`.
...
Set everything up as shown in the image.
...
## Now you can proceed to setting up the first connection.
